...
class ThreadLocalSingletonMeta(type):
    ...
    """
    tls = threading.local()
    _locals_key: str = "singleton_instances"
    ...
    def __call__(cls, *args, **kwargs) -> object:
        cls._init_local()
        if cls.__name__ not in cls.tls.singleton_instances:
            cls.tls.singleton_instances[cls.__name__] = super(ThreadLocalSingletonMeta, cls).__call__(*args, **kwargs)
        return cls.tls.singleton_instances[cls.__name__]
